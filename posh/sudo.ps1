# An approximation of the Unix sudo command.
# Taken from psutils project by Luke Sampson.
# https://github.com/lukesampson/psutils/blob/master/sudo.ps1
if(!$args) { "usage: sudo <cmd...>"; exit 1 }

function is_admin {
    $admin_group = (Get-WmiObject win32_group -filter "LocalAccount=True AND SID='S-1-5-32-544'").name # be language-agnostic
    $admins = (net localgroup $admin_group) -join "`n" | Select-String '(?s)-+\n(.*)' |% { $_.matches.groups[1].value }
